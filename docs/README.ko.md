...
플러그인 업데이트 중에도 개인 스타일을 안전하게 보관하려면 개인 JSON 파일을 여기에 넣으세요:

`ComfyUI/user/default/styler_pipeline/`

- 아래에 표시된 JSON 형식을 사용해 이 폴더에 하나 이상의 `*.json` 파일을 넣습니다.
- 기존 카테고리에 스타일을 추가하려면 포함된 카테고리와 같은 파일 이름을 사용하세요. 예: `mood.json`
- 새 카테고리를 만들려면 새 파일 이름을 사용하세요. 예: `cinematic_portrait.json`
- 포함된 스타일이 먼저 로드되고, 그 다음 개인 스타일이 추가됩니다. 같은 카테고리 안에 같은 `name`의 스타일이 있으면 개인 버전이 사용됩니다.

이 폴더는 플러그인의 포함된 `data/` 폴더가 아니라 `ComfyUI/user/default/` 아래에 있으므로, 플러그인을 업데이트해도 사용자 지정 스타일이 교체되지 않습니다. 호환성을 위해 이전 경로인 `ComfyUI/user/styler_pipeline/*.json`에 이미 놓인 파일도 당분간 계속 로드됩니다.
...
styler 노드가 예측 가능하도록 사용자 지정 `*.json` 스타일 파일의 일관성을 유지하세요.
