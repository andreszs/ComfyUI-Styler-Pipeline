...
- Режим `Randomize` для каждой категории в Browse, чтобы при каждом запуске workflow выбирать другой стиль из выбранных категорий.
...
Используйте chip `Randomize` в любой категории, чтобы при каждой постановке workflow в очередь выбирать случайный стиль из этой категории.

**Примечание:** Для воспроизводимости сгенерированные изображения сохраняют выбранный стиль в метаданных встроенного workflow. При повторной загрузке workflow из сгенерированного изображения `Randomize` больше не будет активен для этой категории; workflow будет использовать именно тот стиль, который был случайно выбран для этой генерации. Выбор конкретного стиля также отключает `Randomize` для этой категории.
...
Чтобы ваши собственные стили не потерялись при обновлении плагина, размещайте личные JSON-файлы здесь:

`ComfyUI/user/default/styler_pipeline/`

- Поместите в эту папку один или несколько файлов `*.json`, используя ту же структуру JSON, которая показана ниже.
- Чтобы добавить стили в существующую категорию, используйте то же имя файла, что и у встроенной категории, например `mood.json`.
- Чтобы создать новую категорию, используйте новое имя файла, например `cinematic_portrait.json`.
- Встроенные стили загружаются первыми, затем добавляются ваши личные стили. Если в вашем файле есть стиль с тем же `name` в той же категории, будет использована ваша версия.

Так как эта папка находится в `ComfyUI/user/default/`, а не во встроенной папке плагина `data/`, ваши пользовательские стили не заменяются при обновлении плагина. Для совместимости файлы, уже размещённые в старом пути `ComfyUI/user/styler_pipeline/*.json`, пока тоже загружаются.
...
Держите пользовательские файлы стилей `*.json` согласованными, чтобы styler nodes оставались предсказуемыми.
