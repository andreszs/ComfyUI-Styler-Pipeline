...
- Browse のカテゴリ別 `Randomize` モードにより、ワークフロー実行ごとに選択カテゴリから別のスタイルを選べます。
...
任意のカテゴリで `Randomize` チップを使うと、ワークフローをキューに入れるたびにそのカテゴリからランダムなスタイルが選ばれます。

**注:** 再現性のため、生成画像には解決済みのスタイルが埋め込み workflow メタデータとして保存されます。生成画像から workflow を再読み込みすると、そのカテゴリの `Randomize` は有効ではなくなり、その生成でランダムに選ばれた正確なスタイルが代わりに使われます。具体的なスタイルを選択した場合も、そのカテゴリの `Randomize` は無効になります。
...
プラグイン更新時に自分のスタイルを失わないように、個人用の JSON ファイルはここに置いてください：

`ComfyUI/user/default/styler_pipeline/`

- 下に示す JSON 形式を使って、このフォルダに 1 つ以上の `*.json` ファイルを置きます。
- 既存カテゴリにスタイルを追加するには、含まれているカテゴリと同じファイル名を使います。例：`mood.json`
- 新しいカテゴリを作るには、新しいファイル名を使います。例：`cinematic_portrait.json`
- 含まれているスタイルが先に読み込まれ、その後に個人用スタイルが追加されます。同じカテゴリ内に同じ `name` のスタイルがある場合は、あなたのバージョンが使われます。

このフォルダはプラグイン内の `data/` フォルダではなく `ComfyUI/user/default/` の下にあるため、プラグイン更新時にカスタムスタイルが置き換えられません。互換性のため、以前の `ComfyUI/user/styler_pipeline/*.json` パスに置かれたファイルも当面は読み込まれます。

**再現性について：** 参照しているスタイル項目がリネーム/削除されない限り、既存のワークフローは再現可能です。古いワークフローが使っているスタイルがリネーム/削除されると、そのワークフローは定義を見つけられず、同じ結果を再現できなくなります。

styler ノードの予測可能性を保つため、カスタム `*.json` スタイルファイルは整合性を維持してください。
